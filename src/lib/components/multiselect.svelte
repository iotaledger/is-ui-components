...
<script>
    ...
    function handleKeyup(e) {
        ...
        if ([38, 40].includes(e.keyCode)) {
            ...
            activeOption =
                calcIndex < 0 ? filtered[filtered.length - 1] : calcIndex === filtered.length ? filtered[0] : filtered[calcIndex]
        }
    }
...
<div class="bg-white border-bottom position-relative">
    <div class="d-flex flex-wrap align-items-center position-relative" class:showOptions on:click={handleTokenClick}>
        {#each Object.values(selected) as s}
            <div class="token d-flex align-items-center rounded mx-1 px-2 my-1 py-2 bg-light" data-id={s.value}>
                <span>{s.name}</span>
                <div
